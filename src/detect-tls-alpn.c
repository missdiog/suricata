...
{
    sigmatch_table[DETECT_AL_TLS_ALPN].name = "tls.alpn";
    sigmatch_table[DETECT_AL_TLS_ALPN].desc = "match TLS ALPN";
    sigmatch_table[DETECT_AL_TLS_ALPN].url = DOC_URL DOC_VERSION "/rules/tls-keywords.html#tls.alpn";
...
#define MAX_SUBSTRINGS 30
    ...
    if (ret != 3) {
        SCLogError(SC_ERR_PCRE_MATCH, "invalid tls.alpn option");
        goto error;
    }
...
/**
 * \brief this function setup the tls.alpn modifier keyword used in the rule
 *
...
/**
 * \test Test that signatures containing a tls.alpn are correctly parsed
...
static int DetectTlsAlpnClientTest01(void)
{
    ...
    de_ctx->sig_list = SigInit(de_ctx, "alert tls any any -> any any "
                               "(msg:\"Testing client tls.alpn\"; "
                               "tls.alpn:client:h2; sid:1;)");
...
static int DetectTlsAlpnServerTest02(void)
{
    ...
    de_ctx->flags |= DE_QUIET;
    de_ctx->sig_list = SigInit(de_ctx, "alert tls any any -> any any "
                               "(msg:\"Testing server tls.alpn\"; "
                               "tls.alpn:server:h2; sid:1;)");
    FAIL_IF_NULL(de_ctx->sig_list);
...
/**
 * \test Test that tls.alpn with a bad handshake side
 */
static int DetectTlsAlpnBadSideTest03(void)
{
    ...
    de_ctx->flags |= DE_QUIET;
    de_ctx->sig_list = SigInit(de_ctx, "alert tls any any -> any any "
                               "(msg:\"Testing trudy tls.alpn\"; "
                               "tls.alpn:trudy:h2; sid:1;)");
