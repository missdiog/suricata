...
//
/* prototypes */
static int DetectIRCSetup (DetectEngineCtx *, Signature *, char *);
void IRCRegisterTests(void);

int DetectAppLayerIRCMatch (ThreadVars *, DetectEngineThreadCtx *,
                                   Flow *, uint8_t , void *,
                                   Signature *, SigMatch *);
void DetectIRCFree(void *);

/**
 * \brief Registration function for uricontent: keyword
 */
void DetectIRCRegister (void)
{
    sigmatch_table[DETECT_IRC].name = "irc";
    sigmatch_table[DETECT_IRC].AppLayerMatch = NULL;
    sigmatch_table[DETECT_IRC].Match = NULL;
    sigmatch_table[DETECT_IRC].Setup = DetectIRCSetup;
    sigmatch_table[DETECT_IRC].Free  = DetectIRCFree;
    sigmatch_table[DETECT_IRC].RegisterTests = HttpUriRegisterTests;

    sigmatch_table[DETECT_IRC].flags |= SIGMATCH_PAYLOAD;
}
//
///**
