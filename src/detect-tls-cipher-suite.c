 ...
 *
 * Implements support for tls.cipher.suite keyword.
 */
...
{
    SCEnter();
    DetectTlsCipherSuiteData *data = (DetectTlsCipherSuiteData *)m->ctx;
    ...
    if (connp->cipher_suites_length > 0 && data->length > 0) {
        for (uint32_t j=0; j < data->length; ++j) {
            for (uint32_t i=0; i < connp->cipher_suites_length; ++i) {
#if __BYTE_ORDER == __BIG_ENDIAN
                ...
                SCLogDebug("** CHECK ** (%d) I:[%04X]/%d:%d J:[%04X]/%d:%d",
                        data->direction, I, i, connp->cipher_suites_length,
                        J, j, data->length);
                if (I == J) {
...
/**
 * \brief Registration function for keyword: tls.cipher.suite
 */
void DetectTlsCipherSuiteRegister(void)
{
    sigmatch_table[DETECT_AL_TLS_CIPHER_SUITE].name = "tls.cipher.suite";
    ...
    sigmatch_table[DETECT_AL_TLS_CIPHER_SUITE].Match = NULL;
    sigmatch_table[DETECT_AL_TLS_CIPHER_SUITE].AppLayerMatch = DetectTlsCipherSuiteMatch;
...
#define MAX_SUBSTRINGS 30
    ...
    SCLogInfo("LENGTH: %d", data->length);
...
/**
 * \test Test that signatures containing a tls.cipher.suite are correctly parsed
...
static int DetectTlsCipherSuiteClientTest01(void)
{
    ...
    de_ctx->sig_list = SigInit(de_ctx, "alert tls any any -> any any "
                               "(msg:\"Testing client tls.cipher.suite\"; "
                               "tls.cipher.suite:client:0x0000; sid:1;)");
...
static int DetectTlsCipherSuiteServerTest02(void)
{
    ...
    de_ctx->flags |= DE_QUIET;
    de_ctx->sig_list = SigInit(de_ctx, "alert tls any any -> any any "
                               "(msg:\"Testing server tls.cipher.suite\"; "
                               "tls.cipher.suite:server:h2; sid:1;)");
    FAIL_IF_NULL(de_ctx->sig_list);
...
/**
 * \test Test that tls.cipher.suite with a bad handshake side
 */
static int DetectTlsCipherSuiteBadSideTest03(void)
{
    ...
    de_ctx->flags |= DE_QUIET;
    de_ctx->sig_list = SigInit(de_ctx, "alert tls any any -> any any "
                               "(msg:\"Testing trudy tls.cipher.suite\"; "
                               "tls.cipher.suite:trudy:0x0000; sid:1;)");
...
/**
 * \test Test that match tls.cipher.suite
 */
static int DetectTlsCipherSuiteMatchTest04(void)
{
    ...
    s_c = DetectEngineAppendSig(de_ctx, "alert tls any any -> any any "
                              "(msg:\"Test client tls.cipher.suite\"; "
                              "tls.cipher.suite:client:0xcca8;"
                              "sid:1;)");
    FAIL_IF_NULL(s_c);

    s_s = DetectEngineAppendSig(de_ctx, "alert tls any any -> any any "
                              "(msg:\"Test server tls.cipher.suite\"; "
