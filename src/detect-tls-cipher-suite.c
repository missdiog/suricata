...
static CipherSuite g_suites_http2_blacklist[] = {
    {0x0000, "TLS_NULL_WITH_NULL_NULL" },
    {0x0001, "TLS_RSA_WITH_NULL_MD5"},
    {0x0002, "TLS_RSA_WITH_NULL_SHA"},
    {0x0003, "TLS_RSA_EXPORT_WITH_RC4_40_MD5"},
...
/*
 ...
 perl -ne '/0x(..),0x(..) - ([^\s]+)/; print "  {0x$1$2, \"\"}, \n"'
*/
static CipherSuite g_suites_openssl_SSLv3[] = {
    {0x0001, "TLS_RSA_WITH_NULL_MD5"},
    {0x0002, "TLS_RSA_WITH_NULL_SHA"},
    {0x002C, "TLS_PSK_WITH_NULL_SHA"},
    {0x002D, "TLS_DHE_PSK_WITH_NULL_SHA"},
...
/*
 ...
 perl -ne '/0x(..),0x(..) - ([^\s]+)/; print "  {0x$1$2, \"\"}, \n"'
*/
static CipherSuite g_suites_openssl_TLSv1[] = {
    {0x00AE, "TLS_PSK_WITH_AES_128_CBC_SHA256"},
    {0x00AF, "TLS_PSK_WITH_AES_256_CBC_SHA384"},
    {0x00B0, "TLS_PSK_WITH_NULL_SHA256"},
    {0x00B1, "TLS_PSK_WITH_NULL_SHA384"},
...
/*
 ...
 perl -ne '/0x(..),0x(..) - ([^\s]+)/; print "  {0x$1$2,\"\"}, \n"'
*/
static CipherSuite g_suites_openssl_TLSv1_2[] = {
    {0x003B, "TLS_RSA_WITH_NULL_SHA256"},
    {0x003C, "TLS_RSA_WITH_AES_128_CBC_SHA256"},
    {0x003D, "TLS_RSA_WITH_AES_256_CBC_SHA256"},
    {0x0040, "TLS_DHE_DSS_WITH_AES_128_CBC_SHA256"},
...
typedef struct NamedCipherSuitePack_ {
    CipherSuite *ciphers;
    const char * name;
} NamedCipherSuitePack;

static NamedCipherSuitePack g_named_suites[] = {
    { g_suites, "all" },
    { g_suites_http2_blacklist, "http2_blacklist" },
    { g_suites_openssl_SSLv3,   "openssl_SSLv3" },
    { g_suites_openssl_TLSv1,   "openssl_TLSv1" },
    { g_suites_openssl_TLSv1_2, "openssl_TLSv1.2" },
    { NULL, NULL }
};
...
typedef struct DetectTlsCipherSuiteData_ {
    ...
    uint32_t packs_length;         /** < Number of packs to search */
    uint16_t *packs;               /** < The list of cipher suites named packs*/
                               ...
                               Signature *s, SigMatch *m)
{
    ...
    if (connp->cipher_suites_length > 0 && ( data->length > 0 || data->packs_length > 0 )) {
        for (uint32_t i=0; i < connp->cipher_suites_length; ++i) {
            SCLogDebug("** I=%d/%d **",i, connp->cipher_suites_length);
#if __BYTE_ORDER == __BIG_ENDIAN
            ...
            } // for list of cipher suites

            for (uint32_t j=0; j < data->packs_length ; ++j) {
                SCLogDebug("** J=%d/%d **",j ,data->packs_length);
                uint16_t p = data->packs[j];
                for (uint32_t k=0; g_named_suites[p].ciphers[k].value != 0xFFFF; ++k) {
                    uint16_t J = g_named_suites[p].ciphers[k].value;
                    SCLogDebug("** PACK CHECK ** (%d) I:[%04X]/%d J:[%04X]/%d",
                            data->direction, I, i, J, k);
                    if (I == J) {
                        SCLogDebug("** PACK MATCHED ** (%d) I:[%04X]/%d J:[%04X]/%d",
                                data->direction, I, i, J, k );
                        SCReturnInt(1);
                    }
                } // for list of cipher suites
...
#define MAX_SUBSTRINGS 30
    ...
    data->packs = SCMalloc(sizeof(uint16_t *) * (data->length));
    if(unlikely(data->packs == NULL)) {
        goto side_error;
    }
    memset(data->packs, '\0', sizeof( uint16_t *) * (data->length));

    size_t g_suites_len = sizeof(g_suites) / sizeof(CipherSuite) - 1;
    size_t g_packs_len  = sizeof(g_named_suites) / sizeof(NamedCipherSuitePack) - 1;

    /* Parse cipher suite list */
    char *saved = NULL;
    char *pch = strtok_r((char *)list_str, g_list_delimiter, &saved);
    size_t rule_suite_index = 0;
    size_t rule_pack_index = 0;
    while (pch != NULL) {
        /* Insert value if is hex value 0x0000 */
        if (strlen(pch) == 6 && pch[0] == '0' && pch[1] == 'x'
        ...
        /* Find by pack name */
        for (size_t i=0; i < g_packs_len; ++i) {
            if ( SCMemcmp(pch, g_named_suites[i].name, strlen(g_named_suites[i].name)) == 0 ) {
                data->packs[rule_pack_index] = i;
                ++rule_pack_index;
                break;
            }
        }
    ...
    data->length = rule_suite_index;
    data->packs_length = rule_pack_index;
